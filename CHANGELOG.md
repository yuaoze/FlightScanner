...
- **精准航班监控**：新增按航班号固定监控模式，支持去程/回程航班号与舱位筛选，Dashboard 卡片同步展示航班号标识
- **Dashboard 快捷添加监控**：总览页新增弹窗式添加监控，减少页面跳转
- **周末雷达目的地扩容**：新增福冈、名古屋、冲绳、釜山、清迈、河内、胡志明市、岘港等目的地

### Changed

- **周末雷达预览**：前端始终展示未来 8 个周末选项，未扫描周末也保留 0 deal 状态
- **周末雷达扫描策略**：目的地搜索增加多爬虫 fallback、随机节流与跨周末冷却，降低连续扫描失败概率

### Fixed

- **去哪儿国际单程兜底**：国际单程在 `wwwsearch` 无结果时，自动回退到 `interroundtrip_compare` 路径取回去程数据
- **Cookie 登录兼容性**：修复从 `frontend/` 等目录启动时扫码登录找不到 `scripts` 模块的问题
- 详见 [feature_log/v2.1.0.md](feature_log/v2.1.0.md)
...
- **周末灵感雷达（Weekend Radar）**：自动扫描未来 N 个周末从上海出发的周五晚 → 周日晚往返组合，按历史均价偏离度排序，AI 生成行程文案，一键「锁定价格并监控」转为精准跟踪路线
- 上海时间窗：去程周五 ≥ 19:00 / 落地周六 ≤ 02:00，回程周日 18:00–23:59 / 落地周一 ≤ 02:00
- 新增 `weekend_radar_cache` 表缓存扫描结果
- AI 简报价格序列按日聚合取最低价，趋势判断更稳定
- 详见 [feature_log/v1.5.0.md](feature_log/v1.5.0.md)
